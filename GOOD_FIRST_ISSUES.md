...
`scripts/from-issue.sh` writes `CHANGE_REQUEST.md` or updates `README.md`
...
Add optional bash and zsh completion scripts for `agentic-workflow.sh`,
...
- [ ] `completions/agentic-workflow.bash` and `.zsh` exist.
...
`agentic-workflow.sh` records stages as `requirements`, `project-plan`, etc.
...
`agentic-workflow.sh` / `change-workflow.sh`. Create a single reference page
...
`agentic-workflow.sh` and `change-workflow.sh` currently expect no arguments.
...
- [ ] `./scripts/agentic-workflow.sh --help` prints usage and exits 0.
- [ ] `./scripts/agentic-workflow.sh --version` prints a version and exits 0.
