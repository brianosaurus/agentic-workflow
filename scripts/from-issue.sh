...
Fetch a GitHub issue and seed a workflow from it.
  ...
  --new      Replace the project-brief section of README.md for
...
EOF
    ...
    # Replace the project-brief section (from '# Project brief' to EOF) in README.md.
    if ! grep -q '^# Project brief$' README.md; then
        echo "README.md does not contain a '# Project brief' section; cannot seed new-app workflow."
        exit 1
    fi
    local head
    head="$(awk '/^# Project brief$/{exit} {print}' README.md)"
    printf '%s\n%s\n' "$head" "$brief" > README.md
    echo "Updated README.md project brief from issue $OWNER/$REPO#$ISSUE_NUM"
    echo "Run: ./scripts/agentic-workflow.sh"
}
